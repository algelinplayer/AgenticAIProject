...
class InstrumentedRAGAgent(RAGKnowledgePromptAgent):
    def get_embedding(self, text):
        print(f"[API] Solicitando embedding para chunk de {len(text)} chars...", flush=True)
...
def main():
    print("[REAL] Carregando .env...", flush=True)
    load_dotenv()
    openai_api_key = os.getenv("OPENAI_API_KEY_DEV") or os.getenv("OPENAI_API_KEY")
    if not openai_api_key:
        print("[REAL] ERRO: OPENAI_API_KEY não encontrada.", flush=True)
        return

    print("[REAL] Inicializando agente com API real...", flush=True)
    persona = "You are a college professor, your answer always starts with: Dear students,"
    # Usando chunks maiores para reduzir número de chamadas de API no teste
    agent = InstrumentedRAGAgent(openai_api_key, persona, chunk_size=1000, chunk_overlap=100)
...
"""
    ...
    print("[REAL] Executando chunk_text...", flush=True)
    agent.chunk_text(knowledge_text)
    print(f"[REAL] Total de chunks: {len(agent.chunks)}", flush=True)

    print("[REAL] Calculando embeddings via API (com timeout)...", flush=True)
    try:
        agent.calculate_embeddings()
        print("[REAL] Embeddings calculados com sucesso.", flush=True)
    except Exception as e:
        print(f"[REAL] ERRO na API de Embeddings: {e}", flush=True)
        return

    prompt = "What is the podcast that Clara hosts about?"
    print(f"[REAL] Prompt: {prompt}", flush=True)
    try:
        # Agora usando use_chat=True para validar também o chat completion
        answer = agent.find_prompt_in_knowledge(prompt, use_chat=True)
        print(f"[REAL] Resposta final:\n{answer}", flush=True)
    except Exception as e:
        print(f"[REAL] ERRO na API de Chat: {e}", flush=True)
